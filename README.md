...
- 화면과 검색·공유 메타정보는 리그 약칭이나 구단 별칭을 쓰지 않으며, 구단은 두 언어 모두 기업명만 표시합니다. API가 전체 구단명을 보내더라도 `teamName`이 표시할 이름을 정규화합니다.
- 선수 응답의 `name`은 한국어 원본이며 `nameEn`, `nameEnSource`가 추가됩니다. 영어 화면은 `nameEn`을 표시하며, 구버전 백엔드에 연결하면 한국어 이름으로 대체합니다.
- `nameEnSource`는 `KBO_OFFICIAL`(공식 영문 프로필) 또는 `ROMANIZED`(공식 표기가 없을 때 생성한 로마자 표기)입니다. 로마자 표기는 선수가 사용하는 공식 철자와 다를 수 있습니다.
