...
상단 언어 링크로 게임 중에도 한국어·영어를 전환할 수 있습니다. `/`은 한국어, `/en/`은 영어 진입점이며 브라우저 언어나 저장된 선택으로 홈의 언어·검색 대표 주소를 바꾸지 않습니다. `?lang=ko` 또는 `?lang=en`으로 명시한 기존 링크도 지원하며, 홈에서는 해당 언어 경로로 정리하고 중복 `lang` 쿼리를 제거합니다. 게임 경로에서는 명시한 언어, 저장된 선택, 브라우저 언어 순서로 결정하며 전환 시 게임 ID와 기록을 유지합니다. 각 홈은 검색·공유 서비스도 읽을 수 있는 정적 메타정보와 서로를 가리키는 `hreflang`을 제공합니다.

한국어 검색 노출 회귀는 영어 브라우저에서 `/`의 본문·제목·canonical을 영어와 `/en/`으로 변경하던 자동 선택 로직 때문이었습니다. 배포 후 Google Search Console의 URL 검사에서 `https://nu-kya.com/`을 실시간 테스트해 렌더링된 한국어 제목·본문과 canonical을 확인하고 색인 생성을 요청하세요. `https://nu-kya.com/sitemap.xml`도 제출할 수 있습니다. 실제 색인 및 검색 결과 갱신 시점은 Google 재수집에 따라 달라집니다.
