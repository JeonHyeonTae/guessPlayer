...
import { ClueExample, SiteFooter } from "./SiteContent";
import {
  LanguageProvider,
  LanguageToggle,
  useI18n,
...
const MAX_TRIES = 9;
const fields: Array<
  [keyof Guess["compare"], TranslationKey, keyof PickedPlayer]
> = [
  ["team", "구단", "team"],
  ["backNo", "등번호", "backNo"],
  ["position", "포지션", "position"],
  ["throwingHand", "투구", "throwingHand"],
  ["battingSide", "타석", "battingSide"],
  ["birthYear", "출생연도", "birthYear"],
  ["height", "키", "height"],
  ["weight", "몸무게", "weight"],
];
...
const resultEmoji: Record<Status, string> = {
  MATCH: "🟩",
  MISMATCH: "⬜️",
  UP: "🟨",
  DOWN: "🟨",
...
function displayField(
  ...
  if (key === "throwingHand" || key === "battingSide")
    return handName(String(value), locale);
  return String(value);
}
...
function StaffToggle({
  ...
  const { t } = useI18n();
  return (
    <div className={modal ? "modal-staff-toggle" : "staff-toggle"}>
      {modal && <b className="modal-setting-title">{t("출제 대상")}</b>}
      <div className="staff-options">
        <button
          className={!includeStaff ? "selected" : undefined}
          onClick={() => onChange(false)}
        >
          {t("선수만")}
          <span>{t("감독·코치 제외")}</span>
        </button>
        <button
          className={includeStaff ? "selected" : undefined}
  ...
  return (
    <div className="share-menu">
      <button
        ...
        onClick={onToggle}
        aria-label={t("구단별 게임 링크 공유하기")}
        aria-expanded={isOpen}
        title={isCopied ? t("복사됐습니다") : t("게임 링크 공유하기")}
      >
        <span className="share-figure" aria-hidden="true">
          <i>?</i>
          <span>{t("누크야")}</span>
          <i>?</i>
        </span>
        <b>{isCopied ? t("복사됨") : t("공유")}</b>
      </button>
      {isOpen && (
        <div
          className="share-menu-list"
          role="menu"
          aria-label={t("공유할 구단 선택")}
        >
          <p className="share-menu-title">{t("공유할 구단을 선택하세요")}</p>
          {teams.length > 0 ? (
            teams.map((team) => (
              <button key={team} role="menuitem" onClick={() => onShare(team)}>
                {teamName(team, locale)}
              </button>
            ))
          ) : (
            <span>{t("구단 목록을 불러오는 중…")}</span>
          )}
        </div>
      )}
    </div>
...
function BrandMark({ onClick }: { onClick?: () => void }) {
  ...
  const content = (
    <>
      <span>{t("누크야")}</span>
      <img src="/nukeya-mark.png" alt="" aria-hidden="true" />
    </>
  );
  return onClick ? (
    <button
      ...
      onClick={onClick}
      aria-label={t("누크야! 메인으로")}
    >
      {content}
    </button>
  ) : (
    <div className="brand-mark" aria-label={t("누크야!")}>
      {content}
    </div>
  );
}
        ...
        {meta ? (
          <>
            ...
            <details className="roster-date-help">
              ...
              <span className="roster-date-tooltip" role="note">
                {t(
                  "표시된 기준일의 1군 등록 명단을 기준으로 하며, 매일 전날 명단으로 갱신됩니다.",
                )}
              </span>
            </details>
            ...
            <span>
              {" "}
              · {t("선수 수")}: {meta.playerCount}
              {locale === "ko" ? "명" : ""} · {t("시도")}: {meta.tries}/
              {MAX_TRIES}
            </span>
          </>
        ) : (
...
function RulesModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      className="game-modal-backdrop rules-modal-backdrop"
      onMouseDown={onClose}
    >
      <section
        className="game-modal rules-modal"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="modal-header">
          <div>
            ...
            <h2 id="rules-title">{t("어떻게 맞히나요?")}</h2>
          </div>
        ...
        <p className="rules-intro">
          {t(
            "선수 정보를 비교하며 숨겨진 한국 프로야구 선수를 최대 9번 안에 맞혀보세요.",
          )}
        </p>
        <ol className="rules-steps">
          <li>
            <span>1</span>
            <div>
              <strong>{t("게임 범위를 정해요")}</strong>
              <p>
                {t(
                  "출제 구단과 1군·퓨처스, 감독·코치 포함 여부를 선택하면 정답 선수가 정해집니다.",
                )}
              </p>
            </div>
          </li>
          <li>
            <span>2</span>
            <div>
              <strong>{t("선수를 추측해요")}</strong>
              <p>
                {t(
                  "선수명을 두 글자 이상 입력하고 검색 결과에서 선수를 선택하세요. 명단에서 출제 가능한 선수도 확인할 수 있어요.",
                )}
              </p>
            </div>
          </li>
          <li>
            <span>3</span>
            <div>
              <strong>{t("단서를 비교해요")}</strong>
              <p>
                {t(
                  "구단, 등번호, 포지션, 투구·타석, 출생연도, 키와 몸무게를 보고 다음 선수를 추리하세요.",
                )}
              </p>
            </div>
          </li>
        ...
        <div className="rules-feedback" aria-label={t("결과 표시 설명")}>
          <div>
            <span className="rules-color match">{t("일치")}</span>
            <p>
              <strong>{t("정답과 같아요")}</strong>
              {t("초록색으로 표시됩니다.")}
            </p>
          </div>
          <div>
            <span className="rules-color mismatch">{t("다름")}</span>
            <p>
              <strong>{t("정답과 달라요")}</strong>
              {t("회색으로 표시됩니다.")}
            </p>
          </div>
          <div>
            <span className="rules-color up">↑</span>
            <p>
              <strong>{t("정답 값이 더 커요")}</strong>
              {t("더 높은 숫자를 찾아보세요.")}
            </p>
          </div>
          <div>
            <span className="rules-color down">↓</span>
            <p>
              <strong>{t("정답 값이 더 작아요")}</strong>
              {t("더 낮은 숫자를 찾아보세요.")}
            </p>
          </div>
        </div>
        ...
        <p className="rules-note">
          {t(
            "↑·↓ 표시는 등번호, 출생연도, 키, 몸무게에서 다음 추측의 방향을 알려줍니다.",
          )}
        </p>
      </section>
    </div>
...
function PlayerRoster({ players }: { players: RosterPlayer[] }) {
  ...
  const activeTeam =
    selectedTeam && teams.includes(selectedTeam) ? selectedTeam : teams[0];
  ...
  return (
    <div className="player-roster">
      <div
        className="roster-team-selector"
        role="tablist"
        aria-label={t("구단 선택")}
      >
        {teams.map((team) => (
          <button
            className={team === activeTeam ? "selected" : undefined}
            key={team}
            role="tab"
            aria-selected={team === activeTeam}
      ...
      <section className="roster-team" ref={rosterContentRef}>
        <div className="roster-level">
          {[
            ...positionOrder,
            ...[...new Set(teamPlayers.map((player) => player.position))]
            ...
            const positionPlayers = teamPlayers
              ...
              .sort((a, b) => {
                if (a.rosterLevel !== b.rosterLevel)
                  return a.rosterLevel === "REGULAR" ? -1 : 1;
                return playerName(a, locale).localeCompare(
                  playerName(b, locale),
                  locale,
                );
              });
            return (
              positionPlayers.length > 0 && (
                <div className="roster-position" key={position}>
                  <b>{positionName(position, locale)}</b>
                  <div>
                    {positionPlayers.map((player, index) => (
                      <span
                        className={`roster-player ${player.rosterLevel.toLowerCase()}`}
                        key={`${player.name}-${player.position}-${player.rosterLevel}-${index}`}
                      >
...
function GameApp() {
  ...
  const resultGrid = useMemo(
    () =>
      guesses
        .map((guess) =>
          fields
            .map(([key]) => resultEmoji[guess.compare[key].status])
            .join(""),
        )
        .join("\n"),
    [guesses],
    ...
    () =>
      rosterPlayers.filter(
        (player) =>
          ...
          (gameIncludesStaff ||
            (player.position !== "감독" && player.position !== "코치")),
      ),
  ...
  const canSearch = useMemo(
    () => query.trim().length >= 2 && mode && !finished && !isSubmitting,
    [query, mode, finished, isSubmitting],
  );

  function setMetaFromState(
    state: { rosterDate: string | null; playerCount: number },
  ...
  async function showAnswer(gameVersion = gameVersionRef.current) {
    ...
    if (gameVersion === gameVersionRef.current) {
      setAnswer(result);
  ...
  async function start(nextMode: Mode) {
    ...
    try {
      ...
      window.history.replaceState(
        null,
        "",
        localizedPath(`/game/${game.gameId}`, localeRef.current),
      );
      setMode(game.mode);
      ...
      if (gameVersion === gameVersionRef.current)
        setMessage(
          "새 게임을 시작하지 못했습니다. 백엔드 연결을 확인해주세요.",
        );
    } finally {
  ...
  async function selectPlayer(player: Player) {
    ...
    try {
      ...
      if (gameVersion !== gameVersionRef.current) return;
      if (result.isCorrect) {
        await showAnswer(gameVersion);
        if (gameVersion === gameVersionRef.current) setMessage("정답입니다!");
      } else if (nextCount >= MAX_TRIES) {
        await showAnswer(gameVersion);
        if (gameVersion === gameVersionRef.current)
          setMessage("아쉽지만, 이번 문제는 여기까지예요.");
      }
    } catch {
      setMessage("추리 요청에 실패했습니다. 잠시 후 다시 시도해주세요.");
    } finally {
  ...
  async function openRoster() {
    ...
    try {
      ...
      setRosterError(
        "선수 명단을 불러오지 못했습니다. 잠시 후 다시 시도해주세요.",
      );
    } finally {
    ...
    api
      ...
      .catch(() => {
        ...
        window.history.replaceState(
          null,
          "",
          localizedPath("/", localeRef.current),
        );
        setGameId(null);
        setMode(null);
      });
    ...
    api
      ...
      .then((teams) => {
        ...
        setSelectedTeams((previous) =>
          previous.filter((team) => teams.includes(team)),
        );
      })
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(
      () =>
        api
          .search(query.trim(), mode, gameIncludesStaff, gameTeams)
          .then((results) => {
            if (!active) return;
            setPlayers(results);
  ...
  useEffect(() => {
    const focusSearchOnTyping = (event: KeyboardEvent) => {
      if (finished || !mode || event.ctrlKey || event.metaKey || event.altKey)
        return;
  ...
  if (isUpdateWindow)
    return (
      <main className="daily-update">
        <LanguageToggle />
        <section>
          <p className="eyebrow">DAILY ROSTER UPDATE</p>
          <h1>{t("선수단 정보를 업데이트하고 있습니다.")}</h1>
          <p>
            {t(
              "매일 새벽 4:00~4:05에는 최신 선수 정보 반영을 위해 잠시 이용할 수 없습니다.",
            )}
          </p>
        </section>
      </main>
    ...
    return (
      <main className="landing">
        ...
        <ShareMenu
          ...
          onToggle={() => setIsShareMenuOpen((value) => !value)}
          onShare={copyShareLink}
        />
        <RulesButton onClick={() => setIsRulesOpen(true)} />
        <button
          aria-label={t(isDarkMode ? "라이트 모드" : "다크 모드")}
          title={t(isDarkMode ? "라이트 모드" : "다크 모드")}
          className="theme-toggle"
          onClick={() => setIsDarkMode((value) => !value)}
        >
          {isDarkMode ? t("라이트 모드") : t("다크 모드")}
        </button>
        <section>
          <BrandMark />
          <h1>
          ...
          <div className="mode-grid">
            <button
              ...
              onClick={() => setSetupMode("REGULAR")}
            >
              {t("1군")}
              <span>{t("현역 1군 선수")}</span>
            </button>
            <button
              className={setupMode === "ALL" ? "selected" : undefined}
              onClick={() => setSetupMode("ALL")}
            >
              {t("1군 + 퓨처스")}
              <span>{t("더 넓은 로스터")}</span>
            </button>
          </div>
          <StaffToggle includeStaff={includeStaff} onChange={setIncludeStaff} />
          <div className="team-picker">
            ...
            <div className="team-list">
              {teamOptions.map((team) => (
                <button
                  className={
                    selectedTeams.includes(team) ? "selected" : undefined
                  }
                  key={team}
                  onClick={() =>
                    setSelectedTeams((previous) =>
                      previous.includes(team)
                        ? previous.filter((value) => value !== team)
                        : [...previous, team],
                    )
                  }
                >
          ...
          <button
            ...
            disabled={selectedTeams.length === 0 || isStartingGame}
            onClick={() => start(setupMode)}
          >
            {isStartingGame
              ? t("게임 시작 중…")
              : t("선택한 구단으로 시작하기")}
  ...
  return (
    <main className="app">
      ...
      <section className="hero">
        <p className="eyebrow">
          {mode === "REGULAR" ? "REGULAR ROSTER" : "ALL ROSTER"}
        </p>
        <div className="hero-title-row">
          <h1>{t("선수 맞추기")}</h1>
          <button
            className="game-action action-new"
            disabled={isStartingGame}
            onClick={() => start(mode)}
            aria-label={t("새 게임 시작")}
            title={t("새 게임 시작")}
          >
            <span aria-hidden="true">↻</span>
          </button>
          <button className="game-action action-roster" onClick={openRoster}>
            {t("명단")}
          </button>
          <button
            className="game-action action-teams"
            onClick={() => setIsTeamPickerOpen(true)}
          >
            {t("구단 선택")}
          </button>
        </div>
        <GameStatus meta={meta} message={message} finished={finished} />
      </section>
      <section className="board">
        ...
        <div className="board-rows" ref={boardRowsRef}>
          {guesses.length === 0 && (
          ...
          {guesses.map((guess, index) => (
            <div className="grid row" key={`${guess.picked.id}-${index}`}>
              <strong>
                {playerName(guess.picked, locale)}
                <small>{teamName(guess.picked.team, locale)}</small>
              </strong>
              {fields.map(([key, , valueKey]) => (
                <div
                  className={`cell ${guess.compare[key].status.toLowerCase()}`}
                  key={key}
                >
                  {displayField(guess.picked, valueKey, locale)}
                  <em>{statusText[guess.compare[key].status]}</em>
                </div>
              ))}
            </div>
          ))}
      ...
      <section className="current-teams">
        <b>{t("현재 출제 구단")}</b>
        <div>
          {gameTeams.map((team) => (
            <span key={team}>{teamName(team, locale)}</span>
          ))}
        </div>
      </section>
          ...
          >
            <input
              ...
              onKeyDown={(event) => {
                ...
                } else if (event.key === "Enter" && activePlayerIndex >= 0) {
                  event.preventDefault();
                  selectPlayer(players[activePlayerIndex]);
                }
              }}
              aria-label={t("선수명 2글자 이상 입력")}
              placeholder={t("선수명 2글자 이상 입력")}
            />
            {canSearch &&
              isSearchFocused &&
              searchStatus !== "ready" &&
              searchStatus !== "idle" && (
                <div className="suggestions search-feedback" role="status">
                  {t(
                    searchStatus === "loading"
                      ? "선수 검색 중…"
                      : searchStatus === "empty"
                        ? "선수를 찾지 못했습니다. 다른 이름으로 검색해주세요."
                        : "선수를 검색하지 못했습니다. 잠시 후 다시 시도해주세요.",
                  )}
                </div>
              )}
            {players.length > 0 && (
              <div className="suggestions" aria-label={t("선수 검색 결과")}>
                {players.map((player, index) => (
                  <button
                    disabled={isSubmitting}
                    className={
                      index === activePlayerIndex ? "active" : undefined
                    }
                    ref={
                      index === activePlayerIndex ? activePlayerRef : undefined
                    }
                    key={player.id}
                    onClick={() => selectPlayer(player)}
                  >
                    <b>{playerName(player, locale)}</b>
                    <span>
                      {teamName(player.team, locale)} ·{" "}
                      {positionName(player.position, locale)} ·{" "}
                      {locale === "ko"
                        ? `${player.birthYear}년생`
      ...
      {answer && isAnswerModalOpen && (
        <div
          ...
          onMouseDown={() => setIsAnswerModalOpen(false)}
        >
          <section
            className="game-modal answer-modal"
            role="dialog"
            aria-modal="true"
            aria-label={t("게임 결과")}
            onMouseDown={(event) => event.stopPropagation()}
          >
            ...
            <div className="answer">
              <p>{t("정답 선수")}</p>
              <h2>{playerName(answer, locale)}</h2>
              <span>
                {teamName(answer.team, locale)} ·{" "}
                {locale === "ko" ? `${answer.backNo}번` : `#${answer.backNo}`} ·{" "}
                {positionName(answer.position, locale)}
              </span>
              <span>
                {answer.height == null ? "—" : `${answer.height} cm`} /{" "}
                {answer.weight == null ? "—" : `${answer.weight} kg`}
              </span>
              <button className="result-copy" onClick={copyResult}>
                {isResultCopied ? t("결과 복사됨") : t("결과 복사하기")}
              </button>
            </div>
            <div className="result-actions">
              <button
                className="result-new-game"
                disabled={isStartingGame}
                onClick={() => start(mode)}
              >
                {isStartingGame ? t("시작 중…") : t("새 게임")}
              </button>
              <button
                className="result-team-select"
                onClick={() => {
                  setIsAnswerModalOpen(false);
      ...
      {isRosterOpen && (
        <div
          className="game-modal-backdrop"
          onMouseDown={() => setIsRosterOpen(false)}
        >
          <section
            className="game-modal roster-modal"
            role="dialog"
            aria-modal="true"
            aria-label={t("전체 선수 명단")}
            onMouseDown={(event) => event.stopPropagation()}
          >
            <div className="modal-header">
              <div>
            ...
            <div className="roster-guide">
              <span>{t("현재 게임에서 선택한 구단의 선수 명단입니다.")}</span>
              <span>{t("구단을 고르면 포지션별로 확인할 수 있어요.")}</span>
              <span className="roster-legend">
                <i className="regular" /> {t("1군 선수")}{" "}
                <i className="futures" />
                {t("퓨처스 선수")}
              </span>
            </div>
            {isRosterLoading ? (
              ...
              <div className="notice">{t(rosterError)}</div>
            ) : (
              <PlayerRoster players={visibleRosterPlayers} />
            )}
          </section>
        </div>
      )}
      {isTeamPickerOpen && (
        <div
          className="game-modal-backdrop"
          onMouseDown={() => setIsTeamPickerOpen(false)}
        >
          <section
            className="game-modal"
            role="dialog"
            aria-modal="true"
            aria-label={t("다음 게임 설정")}
            onMouseDown={(event) => event.stopPropagation()}
          >
            <div className="modal-header">
              <div>
            ...
            <p>
              {t(
                "게임을 시작하면 아래 설정으로 새로운 정답 선수가 출제됩니다.",
              )}
            </p>
            ...
            <StaffToggle
              ...
              onChange={setIncludeStaff}
              modal
            />
            <div className="modal-team-picker">
              <b>{t("출제 구단")}</b>
              <button
                onClick={() =>
                  setSelectedTeams(
            ...
            <button
              className="modal-start-game"
              disabled={selectedTeams.length === 0 || isStartingGame}
              onClick={() => start(setupMode)}
            >
              {isStartingGame ? t("게임 시작 중…") : t("새 게임 시작")}
            </button>
          </section>
        </div>
      )}
