...
function RulesModal({ onClose }: { onClose: () => void }) {
  ...
  return <div className="game-modal-backdrop rules-modal-backdrop" onMouseDown={onClose}>
    <section className="game-modal rules-modal" role="dialog" aria-modal="true" aria-labelledby="rules-title" onMouseDown={event => event.stopPropagation()}>
      ...
      <ClueExample en={locale === 'en'} /><p className="rules-note">{t('↑·↓ 표시는 등번호, 출생연도, 키, 몸무게에서 다음 추측의 방향을 알려줍니다.')}</p>
    </section>
  </div>
}
...
function GameApp() {
  ...
  if (!mode) return <main className="landing"><LanguageToggle /><ShareMenu teams={shareTeams} isOpen={isShareMenuOpen} isCopied={isShareCopied} onToggle={() => setIsShareMenuOpen(value => !value)} onShare={copyShareLink} /><RulesButton onClick={() => setIsRulesOpen(true)} /><button aria-label={t(isDarkMode ? '라이트 모드' : '다크 모드')} title={t(isDarkMode ? '라이트 모드' : '다크 모드')} className="theme-toggle" onClick={() => setIsDarkMode(value => !value)}>{isDarkMode ? t('라이트 모드') : t('다크 모드')}</button><section><BrandMark /><h1>{t('한국 프로야구')}<br />{t('선수를 맞혀보세요.')}</h1><p>{locale === 'en' ? 'Choose your teams. Find the player in nine guesses.' : '구단을 고르고, 9번 안에 선수를 맞혀보세요.'}</p><AdSenseBanner className="landing-ad" teams={selectedTeams} /><div className="mode-grid"><button className={setupMode === 'REGULAR' ? 'selected' : undefined} onClick={() => setSetupMode('REGULAR')}>{t('1군')}<span>{t('현역 1군 선수')}</span></button><button className={setupMode === 'ALL' ? 'selected' : undefined} onClick={() => setSetupMode('ALL')}>{t('1군 + 퓨처스')}<span>{t('더 넓은 로스터')}</span></button></div><StaffToggle includeStaff={includeStaff} onChange={setIncludeStaff} /><div className="team-picker"><div><b>{t('출제 구단')}</b><button onClick={() => setSelectedTeams(selectedTeams.length === teamOptions.length ? [] : teamOptions)}>{selectedTeams.length === teamOptions.length ? t('전체 해제') : t('전체 선택')}</button></div><div className="team-list">{teamOptions.map(team => <button className={selectedTeams.includes(team) ? 'selected' : undefined} key={team} onClick={() => setSelectedTeams(previous => previous.includes(team) ? previous.filter(value => value !== team) : [...previous, team])}>{teamName(team, locale)}</button>)}</div></div><button className="start-game" disabled={selectedTeams.length === 0 || isStartingGame} onClick={() => start(setupMode)}>{isStartingGame ? t('게임 시작 중…') : t('선택한 구단으로 시작하기')}</button>{message && <div className="notice">{t(message)}</div>}</section><SiteFooter />{isRulesOpen && <RulesModal onClose={() => setIsRulesOpen(false)} />}</main>
  ...
  return (
    <main className="app">
      ...
      <section className="board">
        ...
        <div className="board-rows" ref={boardRowsRef}>{guesses.length === 0 && <p className="first-guess-tip">{locale === 'en' ? 'Search for a player to start. For example, 1995 ↑ means the answer was born after 1995.' : '선수 이름을 검색해 첫 추측을 해보세요. 출생연도 1995 ↑는 정답이 1995년 이후에 태어났다는 뜻이에요.'}</p>}{guesses.map((guess, index) => <div className="grid row" key={`${guess.picked.id}-${index}`}><strong>{playerName(guess.picked, locale)}<small>{teamName(guess.picked.team, locale)}</small></strong>{fields.map(([key, , valueKey]) => <div className={`cell ${guess.compare[key].status.toLowerCase()}`} key={key}>{displayField(guess.picked, valueKey, locale)}<em>{statusText[guess.compare[key].status]}</em></div>)}</div>)}</div>
      </section>
      ...
      <AdSenseBanner className="game-ad" teams={gameTeams} allowGoogleAds={guesses.length > 0 && !!meta && !message && !finished && !isStartingGame && !isRulesOpen && !isRosterOpen && !isTeamPickerOpen} />
      <SiteFooter />
